...
Sensitive fields in OSGi configuration files and in managed objects are protected automatically.
...
Encryption is appropriate when the original value must be recoverable (for example, passwords used in outbound connector authentication).
Hashing is appropriate when only equality verification is required.
...
Bundle developers declare which fields must be protected by implementing `MetaDataProvider`.
The Crypto Service applies encryption automatically at write time based on that metadata.

For built-in components, this metadata is already provided.
...
The shipped `boot.properties` sets `openidm.config.crypto.alias` to `openidm-sym-default`.
The Java-level fallback (used only if the property is absent) is `openidm-config-default`.
...
The protection is applied in the `onStore` lifecycle hook before writing to the repository, and is idempotent.
Already-protected values are not processed again.

=== Field encryption

Add an `encryption` object to the field definition in `managed.json`.
The `key` property is required.
The `cipher` property is optional and defaults to `AES/CBC/PKCS5Padding`.
...
NOTE: Encrypted fields are returned as `$crypto` blobs when the object is read.
Decryption is not applied automatically on retrieval.
...
For guidance on when to use hashing versus encryption, see <<hashing,Hashing>>.
...
Returns a `$crypto` JSON object (see <<encryption,Encryption>> for the wire format).
...
Returns a salted-hash `$crypto` JSON object (see <<hashed-value-wire-format,wire format>>).
...
It remains available because `CryptoServiceImpl.hash()` uses only `MessageDigest`-based `FieldStorageScheme` implementations and requires no keystore access.
It continues to operate via the retained service reference even after unbind.
